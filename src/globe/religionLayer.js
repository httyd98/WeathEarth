/**
 * Religion Layer — world religions choropleth.
 * Extruded country polygons colored by dominant religion.
 * Supports metric filter (show distribution of a specific religion across countries).
 */
...
import { COUNTRY_RELIGIONS, RELIGION_META, dominantReligion } from "../data/religionData.js";

const BAR_MAX_H = 0.6;
const BASE_R    = GLOBE_RADIUS;

// Current display mode:
//   "dominant" → color by majority religion, height = % of majority
//   <key>       → color by intensity of that specific religion across all countries
let _mode = "dominant";
...
// ── Geometry helpers (same pattern as anthropologyLayer.js) ─────────────────
...
function _religionColor(religionKey, alpha = 1) {
  const meta = RELIGION_META[religionKey];
  if (meta) {
    _color.set(meta.color);
  } else {
    _color.set(0xaabbcc);
  }
}
...
function _intensityColor(t) {
  ...
  if (t < 0.5) {
    const u = t / 0.5;
    _color.setRGB(0.3 + u * 0.7, 0.3 + u * 0.7, 0.3 * (1 - u));
  } else {
    const u = (t - 0.5) / 0.5;
    _color.setRGB(1, 1 - u * 0.9, 0);
  }
}
...
function _buildMeshes() {
  ...
  // Precompute per-country value for current mode
  const values = COUNTRIES.map(c => {
    const rel = COUNTRY_RELIGIONS[c[0]];
    if (!rel) return 0;
    if (_mode === "dominant") {
      // height = percentage of dominant religion / 100
      const dom = dominantReligion(rel.pct);
      return (rel.pct?.[dom] ?? 0) / 100;
    } else {
      return (rel.pct?.[_mode] ?? 0) / 100;
    }
  });
  ...
  for (let ci = 0; ci < COUNTRIES.length; ci++) {
    ...
    const height = Math.max(0.004, val * BAR_MAX_H);
...
export async function enableReligion() {
  _active = true;
  if (!_polygonsLoaded) {
...
export function disableReligion() {
  _active = false;
  for (const { mesh } of _meshes) mesh.visible = false;
}

/** Set display mode: "dominant" | religion key from RELIGION_META */
export function setReligionMode(mode) {
  _mode = mode;
  if (_active) _buildMeshes();
}

export function getReligionMode() { return _mode; }

export function getReligionMeshes() {
  return _meshes.map(m => m.mesh);
}

export function getReligionCountry(countryIndex) {
  const c = COUNTRIES[countryIndex];
  if (!c) return null;
  const rel = COUNTRY_RELIGIONS[c[0]];
  const dom = rel ? dominantReligion(rel.pct) : null;
  return {
    code: c[0],
    flag: c[1],
    name: c[2],
    ...
    lon:  c[4],
    dom,
    domLabel: dom ? (RELIGION_META[dom]?.label ?? dom) : "Nessun dato",
    breakdown: rel?.pct ?? {},
  };
}

export { RELIGION_META };
